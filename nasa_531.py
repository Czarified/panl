...
def nasa_5_3_1_validation():
    ...
    bc_type[20:22] = 1; bc_value[20:22] = 0.0
    bc_type[100] = 1; bc_value[100] = 0.0
    
    u, t = solver.solve(bc_type, bc_value)
    
    # Peak Profiling at r=5.05 (1.01*R)
    angles = np.array([0, np.pi/4, np.pi/2])
    pts_circ = np.array([[W/2 + 5.05 * np.cos(a), H/2 + 5.05 * np.sin(a)] for a in angles])
    
    st_circ = solver.compute_stress(pts_circ, u, t)
    print(f"Sigma_xx profile at r=5.05:")
